...
def _derive_original_out_path(generated_path: Path) -> Path:
    stem = generated_path.stem
    if stem.endswith("_generated"):
        stem = stem[: -len("_generated")]
    original_stem = f"{stem}_original"
    return generated_path.with_name(original_stem + generated_path.suffix)
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--out",
        default=None,
        help="Generated-dataset JSONL path (defaults to timestamped freq_blimp file in data/processed).",
    )
    ap.add_argument(
        "--original-out",
        default=None,
