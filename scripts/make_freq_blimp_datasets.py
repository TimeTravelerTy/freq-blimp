...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Generate multiple freq BLiMP datasets for a list of Zipf thresholds."
    )
    ap.add_argument(
        "--zipf_values",
        nargs="+",
        required=True,
        type=float,
        help="One or more Zipf thresholds to pass as --zipf_all to make_freq_blimp_dataset.py.",
    )
    ap.add_argument(
        "--out-dir",
    ...
    script_path = Path(__file__).resolve().parent / "make_freq_blimp_dataset.py"
