...
def _short(rec: dict) -> str:
    ...
    return (
        f"[{rec.get('phenomenon')}|{rec.get('subtask')}|row={rec.get('_row')}|idx={rec.get('idx')}]\n"
        f"  good_original: {g}\n"
        f"  bad_original : {b}\n"
        f"  good_rare   : {gr}\n"
        f"  bad_rare    : {br}\n"
...
def _by_subtask(recs: Iterable[dict]) -> Counter:
    return Counter((r.get("phenomenon"), r.get("subtask")) for r in recs)
...
def spotcheck(records: List[dict], *, sample_k: int, seed: int) -> int:
    ...
    def _print_issue(name: str, recs: List[dict]) -> None:
        ...
        for (phen, subtask), c in counts.most_common(8):
            print(f"  {c:5d}  {phen}|{subtask}")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Spot-check a generated freq-BLiMP JSONL dataset.")
