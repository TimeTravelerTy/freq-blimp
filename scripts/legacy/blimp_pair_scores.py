...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Score BLiMP/freq-BLiMP pairs and save per-pair logprob data."
    )
    ap.add_argument("--data", required=True, help="Path to a BLiMP/freq-BLiMP JSONL dataset.")
    ...
    if args.variant == "both":
        ...
        for variant in ("original", "rare"):
            if not _variant_has_pairs(variant, records, args.good_field, args.bad_field):
                print(f"[Skip] No scorable {variant} pairs found in {args.data}.")
                continue
            _write_variant(scorer, variant, records, args.good_field, args.bad_field, args)
            wrote_any = True
        if not wrote_any:
            raise ValueError("No scorable original or rare pairs found.")
        ...
        if _variant_has_pairs("rare", records, args.good_field, args.bad_field):
            ...
            raise ValueError("No scorable original or rare pairs found.")
        print(f"[Auto] Resolved variant={resolved_variant}")
        _write_variant(scorer, resolved_variant, records, args.good_field, args.bad_field, args)
    else:
